...
const ChatWidget = ({ onBookCallClick }: ChatWidgetProps) => {
  ...
  const [messages, setMessages] = useState([
    { text: "Hi! 👋 I'm here to help you learn about our automation services. Ask me anything!", sender: "bot" }
  ]);
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-40 w-14 h-14 md:w-16 md:h-16 bg-primary hover:bg-primary/90 text-primary-foreground rounded-full shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center group"
          ...
          <div className="p-4 border-t border-border">
            <div className="flex items-center space-x-2">
              <input
                ...
                className="flex-1 bg-muted text-foreground px-4 py-2 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-primary"
              />
              <button
                onClick={handleSend}
                className="bg-primary hover:bg-primary/90 text-primary-foreground w-10 h-10 rounded-full flex items-center justify-center transition-colors"
              >
