      ...
      <div className="container mx-auto px-6">
        ...
        <div ref={cardsRef} className="grid md:grid-cols-2 gap-8 max-w-6xl mx-auto">
          {cases.map((useCase, index) => <div key={index} className={`glass-card p-8 hover-glow transition-all duration-300 hover:scale-[1.02] scroll-animate ${cardsInView ? `${index % 2 === 0 ? 'animate-slide-left-fade' : 'animate-slide-right-fade'} stagger-${Math.floor(index / 2) + 1}` : ''}`}>
              ...
              <button onClick={onBookCallClick} className="w-full bg-background text-primary border-2 border-primary hover:bg-primary/10 py-3 font-semibold transition-all duration-300 flex items-center justify-center space-x-2 group rounded-2xl">
        ...
        <div className="text-center mt-12">
          <button onClick={onBookCallClick} className="inline-flex items-center justify-center bg-primary hover:bg-primary/90 text-primary-foreground px-8 py-6 transition-colors hover:scale-105 duration-300 text-sm font-semibold rounded-3xl"> Book Your Free Strategy Call Now</button>
